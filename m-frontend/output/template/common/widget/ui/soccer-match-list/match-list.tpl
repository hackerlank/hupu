...
<div class="match-list-wrap" id="J_content" data-start-time="{%$startTime|f_escape_xml%}" data-end-time="{%$endTime|f_escape_xml%}">
{%foreach from=$data item=val name=key%}
<section class="match-section {%if $smarty.foreach.key.index == 0%}match-today{%/if%}">
<div class="time-label">
<span>{%$val[0]["date"][0]|f_escape_xml%}&nbsp;{%$val[0]["week"]|f_escape_xml%}</span>
</div>
...
{%/if%}
<div class="match-list {%$matchClassName|f_escape_xml%}">
{%if isset($val1["away_formation"]) && isset($val1["home_formation"])%}
<a href="{%$val1["game_url"]|f_escape_xml%}">
{%else%}
...
{%if $val1["game_status"] == 0%}
{%if isset($val1["away_formation"]) || isset($val1["home_formation"])%}
<div class="match-state">{%$val1["date"][1]|f_escape_xml%}</div>
<div class="match-tag">
<i class="tag-battle">阵容</i>
</div>
{%else%}
<div class="match-state match-state-top">{%$val1["date"][1]|f_escape_xml%}</div>
{%/if%}
{%** 比赛状态 结束 **%}
{%elseif $val1["game_status"] == 30%}
...
{%else%}
{%$val1["game_period_cn"]|f_escape_xml%}<br />
{%$val1["game_time"]|f_escape_xml%}
{%/if%}
</div>
{%if isset($val1["home_score"])%}
<div class="match-score">
{%$val1["home_score"]|f_escape_xml%}{%if isset($val1["home_shoot_out_score"])%}<span class="thepenalty">({%$val1["home_shoot_out_score"]|f_escape_xml%})</span>{%/if%}<br />
{%$val1["away_score"]|f_escape_xml%}{%if isset($val1["away_shoot_out_score"])%}<span class="thepenalty">({%$val1["away_shoot_out_score"]|f_escape_xml%})</span>{%/if%}
</div>
{%/if%}
<div class="team-name">
...
<i class="team-logo">
<img src="{%$val1["home_logo"]|f_escape_xml%}" alt=""/>
</i>
{%/if%}
<h4>{%$val1["home_name"]|f_escape_xml%}</h4>
</div>
<div class="team-name-list">
{%if !empty($val1["away_logo"])%}
<i class="team-logo">
<img src="{%$val1["away_logo"]|f_escape_xml%}" alt=""/>
</i>
{%/if%}
<h4>{%$val1["away_name"]|f_escape_xml%}</h4>
</div>
</div>
<div class="match-played">
{%if !empty($leagueCN)%}{%$leagueCN|f_escape_xml%}<br />{%/if%}
{%if !empty($val1["round_type_str"])%}
{%$val1["round_type_str"]|f_escape_xml%}
{%elseif !empty($val1["match_day"])%}
第{%$val1["match_day"]|f_escape_xml%}轮{%/if%}
</div>
</a>
</div>
{%/foreach%}
        ...
        <section class="match-section">
            ...
            <@ _.each(item, function(item1,index1) { @>
                ...
                <@ } @>
                        ...
                        <div class="match-played">
                            <@=datas.leagueCN@><br />
                            <@ if( !_.isNull( item1.round_type_str ) ) { @>
                                <@=item1.round_type_str@>
                            <@ }else if( !_.isNull( item1.match_day ) ){@>
                                第<@=item1.match_day@>轮
                            <@ } @>
                        </div>
                    </a>
