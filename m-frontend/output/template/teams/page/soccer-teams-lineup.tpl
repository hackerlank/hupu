...
<h3 style="background-color:{%$positionColors[$pos]|f_escape_xml%}">{%$positions[$pos]|f_escape_xml%}</h3>
</li>
{%foreach $players[$pos] as $player%}
<li data-href="{%$player.player_url|f_escape_xml%}">
<span>{%$player.known_name|f_escape_xml%}</span>
<span>{%calc_age birth=$player.birth_date%}</span>
<span class="w1">{%$player.country|default:"--"|f_escape_xml%}</span>
<span>{%$player.jersey_num|default:"--"|f_escape_xml%}</span>
</li>
{%/foreach%}
{%/foreach%}
</ul>
...
{%require name='teams:page/soccer-teams-lineup.tpl'%}{%/block%}
