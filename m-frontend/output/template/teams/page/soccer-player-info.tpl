...
{%widget
        ...
        %}
<nav>
<a href="javascript:void(0);" class="info active">基本信息</a><a href="{%$playerInfo.page_urls.stats_url|f_escape_xml%}">赛季数据</a>
</nav>
...
{%$playerInfo.birth_date|f_escape_xml%}
{%else%}
--{%/if%}
</div>
...
€{%$playerInfo.market_value|f_escape_xml%}万{%else%}
--{%/if%}
</div>
...
{%$playerInfo.age|f_escape_xml%}岁{%else%}
--{%/if%}
</div>
...
{%$playerInfo.height|f_escape_xml%}米{%else%}
--{%/if%}
</div>
...
{%$playerInfo.weight|f_escape_xml%}公斤{%else%}
--{%/if%}
</div>
...
左脚{%else if $playerInfo.preferred_foot ==2%}
右脚{%else if $playerInfo.preferred_foot ==3%}
双脚{%else%}
未知{%/if%}
</div>
...
<div class="contract">（{%$playerInfo.contract_until|f_escape_xml%}&nbsp;&nbsp;合同到期）</div>
{%/if%}
</section>
<section class="mod-data">
<h2 class="mod-title-h2"><span class="icon"></span>赛季综合({%$season|f_escape_xml%})</h2>
...
<div class="icon"></div>
<div class="data-info">
<div class="value">{%$playerStats.goals|default:"--"|f_escape_xml%}/{%$playerStats.att_pen_goal|default:"--"|f_escape_xml%}</div>
...
<div class="icon"></div>
<div class="data-info">
<div class="value">{%$playerStats.goal_assist|default:"--"|f_escape_xml%}</div>
...
<div class="icon"></div>
<div class="data-info">
<div class="value">{%$playerStats.total_pass|default:"--"|f_escape_xml%}</div>
...
<div class="icon"></div>
<div class="data-info">
<div class="value">{%$playerStats.yellow_card|default:"--"|f_escape_xml%}/{%$playerStats.red_card|default:"--"|f_escape_xml%}</div>
...
<div class="icon"></div>
<div class="data-info">
<div class="value">{%$playerStats.played|default:"--"|f_escape_xml%}/{%$playerStats.status|default:"--"|f_escape_xml%}</div>
...
<div class="icon"></div>
<div class="data-info">
<div class="value">{%$playerStats.mins_played|default:"--"|f_escape_xml%}'</div>
...
<div class="icon"></div>
<div class="data-info">
<div class="value">{%$playerStats.total_tackle|default:"--"|f_escape_xml%}</div>
...
<div class="icon"></div>
<div class="data-info">
<div class="value">{%$playerStats.total_clearance|default:"--"|f_escape_xml%}</div>
...
<div class="icon"></div>
<div class="data-info">
<div class="value">{%$playerStats.interception|default:"--"|f_escape_xml%}</div>
...
<div class="icon"></div>
<div class="data-info">
<div class="value">{%$playerStats.saves|default:"--"|f_escape_xml%}</div>
...
<span class="item{%$playerInfo.detailed_position.main_position.type|f_escape_xml%} red">{%$playerInfo.detailed_position.main_position.name_zh|f_escape_xml%}</span>
{%if !empty($playerInfo.detailed_position.side_position)%}
{%foreach $playerInfo.detailed_position.side_position as $val%}
<span class="item{%$val.type|f_escape_xml%}">{%$val.name_zh|f_escape_xml%}</span>
{%/foreach%}
{%/if%}
</div>
</section>
...
<tr class="item">
<td class="date">{%$val.transfer_date|f_escape_xml%}</td>
<td width="50%">
{%if !empty($val.moving_to_team_name_zh)%}
{%$val.moving_to_team_name_zh|f_escape_xml%}
{%else%}
不详{%/if%}
</td>
<td class="fee">
{%if !empty($val.transfer_fee)%}
€{%$val.transfer_fee|f_escape_xml%}万{%else%}
--{%/if%}
</td>
</tr>
{%/foreach%}
...
{%require name='teams:page/soccer-player-info.tpl'%}{%/block%}