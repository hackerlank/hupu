			...
			<ul class="detail">
                ...
                <li>
                    {%if !empty($info.country)%}
                        <span class="country">
                            <span class="flag" style="background-image:url(//b3.hoopchina.com.cn/images/country/{%$info.country%}.jpg);">{%if !empty($kanqiu.night)%}<div class="mask"></div>{%/if%}</span><span class="name">{%$info.country%}</span>
                        </span>
                    {%/if%}
                </li>
                {%else%}
                ...
                <li>
                    ...
                    {%if (!empty($info.positionZhName) && !empty($info.country)) || ($info.number!="" && !empty($info.country))%}
		                 <i class="separator">|</i>
					{%/if%}
                    {%if !empty($info.country)%}
                         <span class="country">
					...
					<div class="info-list">
						<table width="100%">
							...
							<tr>
								...
								<td class="td2" colspan="3">{%$info.draft_year%}年{%$info.draft_round%}轮{%$info.draft_pick%}顺位</td>
							</tr>
							{%/if%}
							...
							{%foreach $rank as $val%}
								...
								{%/if%}
									...
									{%if !empty($val[1]) && $val[1] < 100%}
			                            <div class="rank">
                                            <a href="{%if !empty($is_app)%}{%$val[3]|escape:none%}{%else%}javascript:void(0);{%/if%}">联盟第<span>{%$val[1]%}</span>&nbsp;{%if !empty($is_app)%}>{%/if%}</a>
                                        </div>
									{%/if%}
								</li>
							{%/foreach%}
