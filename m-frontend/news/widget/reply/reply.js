    ...
    var PopupReply = {
        ...
        viewMoreReply: function() {
            ...
            // 未登录只显示亮评论，普通评论隐藏，第一次点击删除class
            if($('.newest-reply-notlogin').length) {
                $('.newest-reply').removeClass('newest-reply-notlogin');
                return;
            }

            //第一次点击显示的是隐藏的内容，第二次点击再请求数据
            if( !this.flag ){
                $('.newest-reply .reply-list').each(function(index, el) {
                ...
                if( this.replyPageNumber + 1 > this.pagecount ){
            ...
            $.ajax({
                ...
                success: function(data) {
                    if ( data.status === 200 ) {
                        ...
                        elem.find('span').html('点亮('+data.light+')');
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if ( data.status === 200 ) {
                        ...
                        that.$reply.find('.newest-reply .reply-inner').prepend( tpl );
                        //that.listBind();
                        that.closePopup();
