...
{%widget name="common:widget/bread-crumbs/bread-crumbs.tpl" title1="NBA比赛中心" title2="{%$match_info.away_name|f_escape_xml%}vs{%$match_info.home_name|f_escape_xml%}文字实录" %}
...
<td width="69">球队</td>
<td width="380">事件</td>
<td width="157" align="center">比分（{%$match_info.away_name|f_escape_xml%}-{%$match_info.home_name|f_escape_xml%}）</td>
</tr>
</table>
</div>
...
<tr id="{%$val.sequence_id|f_escape_xml%}">
<td colspan="4" style="font-weight:bold;" align="center">{%$val.event|f_escape_xml%}</td>
</tr>
{%else%}
<tr id="{%$val.sequence_id|f_escape_xml%}">
<td class="tdw-1 left" width="69">{%$val.left_time|f_escape_xml%}</td>
<td width="69">
{%if $val.team == "home"%}{%$match_info.home_name|f_escape_xml%}{%else%}{%$match_info.away_name|f_escape_xml%}{%/if%}
</td>
<td width="380" {%if $val.event_type==1 || $val.event_type==3 %} style="font-weight: bold;" {%/if%}>{%$val.event|f_escape_xml%}</td>
{%$n=strpos($val.vs,'-')%}
{%$vsHome=substr($val.vs,0,$n)%}
{%$vsAway=substr($val.vs,$n+1)%}
<td width="157" align="center">{%$vsAway|f_escape_xml%}-{%$vsHome|f_escape_xml%}</td>
</tr>
...
{%widget name="match:widget/last-post/last-post.tpl" title="{%$match_info.away_name|f_escape_xml%}" logo="{%$match_info.away_logo|f_escape_xml%}" %}
{%widget name="match:widget/last-post/last-post.tpl" title="{%$match_info.home_name|f_escape_xml%}" logo="{%$match_info.home_logo|f_escape_xml%}" %}
...
{%require name='match:page/playbyplay.tpl'%}{%/block%}
