...
<script type="text/javascript">
        window.GM = {
            websocket: "{%$websocket[$websocket|array_rand]|f_escape_js%}",
            plus: "{%$match_stats.$home_id.plus|f_escape_js%}",
            match_id: "{%$match_info.match_id|f_escape_js%}",
...
{%widget
        ...
        title1="NBA比赛中心"
        title2="{%$match_info.away_name|f_escape_xml%}vs{%$match_info.home_name|f_escape_xml%}数据统计"
    %}
...
<div class="clearfix">
<h2>{%$match_info.away_name|f_escape_xml%}（客队）</h2>
<div class="tips_layes" style="color: rgb(153, 153, 153);">
?<div class="tips_tems">
<div class="post_tips">
</div>
<div class="fonts">
+/- ：指球员在场上时球队的净胜分<br />PG ：控球后卫<br />SG ：得分后卫<br />G ：后卫<br />SF ：小前锋<br />PF ：大前锋<br />F ：前锋<br />G/F ：后卫/前锋<br />C ：中锋<br />F/C ：前锋/中锋<br />
</div>
</div>
</div>
</div>
...
<td class="tdw-1 left"><a href="{%$val.player_link|f_escape_xml%}" target="_blank">{%$val.name|f_escape_xml%}</a></td>
...
<span class="high">{%$val.reb|f_escape_xml%}</span>{%else%}{%$val.reb|f_escape_xml%}{%/if%}
</td>
<td>
{%if $val.asts == $player_stats.away_max_data.asts && $val.asts != 0%}
<span class="high">{%$val.asts|f_escape_xml%}</span>{%else%}{%$val.asts|f_escape_xml%}{%/if%}
</td>
...
<span class="high">{%$val.pts|f_escape_xml%}</span>{%else%}{%$val.pts|f_escape_xml%}{%/if%}
</td>
<td>
{%if $val.net_points == $player_stats.away_max_data.net_points && $val.net_points != 0%}
<span class="high">{%if $val.net_points > 0 %}+{%/if%}{%$val.net_points|f_escape_xml%}</span>
{%else%}
{%if $val.net_points > 0 %}+{%/if%}{%$val.net_points|f_escape_xml%}
{%/if%}
</td>
</tr>
{%/foreach%}
...
<td>
{%if $val.net_points == $player_stats.away_max_data.net_points && $val.net_points != 0%}
<span class="high">{%if $val.net_points > 0 %}+{%/if%}{%$val.net_points|f_escape_xml%}</span>{%else%}{%if $val.net_points > 0 %}+{%/if%}{%$val.net_points|f_escape_xml%}{%/if%}
</td>
</tr>
{%/foreach%}
<tr class="title bg_a" style="background-color: rgb(251, 251, 251);">
...
<td>&nbsp;</td>
<td>&nbsp;</td>
<td>{%(($awaySTfgm+$awayREfgm)*100/($awaySTfga+$awayREfga))|string_format:"%.1f"%}%</td>
<td>{%(($awaySTtpm+$awayREtpm)*100/($awaySTtpa+$awayREtpa))|string_format:"%.1f" %}%</td>
<td>{%(($awaySTftm+$awayREftm)*100/($awaySTfta+$awayREfta))|string_format:"%.1f"%}%</td>
<td>&nbsp;</td>
<td>&nbsp;</td>
<td>&nbsp;</td>
<td>&nbsp;</td>
...
<div class="table_list_live">
<div class="clearfix">
<h2>{%$match_info.home_name|f_escape_xml%}（主队）</h2>
<div class="tips_layes" style="color: rgb(153, 153, 153);">
...
<td>
{%$homeSTtpm=0%}{%$homeREtpm=0%}{%foreach from=$player_stats.away_start item=val name=key%}{%$homeSTtpm=$homeSTtpm+$val.tpm%}{%/foreach%}{%foreach from=$player_stats.away_reserve item=val name=key%}{%$homeREtpm=$homeREtpm+$val.tpm%}{%/foreach%}
{%$homeSTtpa=0%}{%$homeREtpa=0%}{%foreach from=$player_stats.away_start item=val name=key%}{%$homeSTtpa=$homeSTtpa+$val.tpa%}{%/foreach%}{%foreach from=$player_stats.away_reserve item=val name=key%}{%$homeREtpa=$homeREtpa+$val.tpa%}
{%/foreach%}{%$homeSTtpm+$homeREtpm%}-{%$homeSTtpa+$homeREtpa%}
</td>
<td>
{%$homeSTftm=0%}{%$homeREftm=0%}{%foreach from=$player_stats.away_start item=val name=key%}{%$homeSTftm=$homeSTftm+$val.ftm%}{%/foreach%}{%foreach from=$player_stats.away_reserve item=val name=key%}{%$homeREftm=$homeREftm+$val.ftm%}{%/foreach%}
{%$homeSTfta=0%}{%$homeREfta=0%}{%foreach from=$player_stats.away_start item=val name=key%}{%$homeSTfta=$homeSTfta+$val.fta%}{%/foreach%}{%foreach from=$player_stats.away_reserve item=val name=key%}{%$homeREfta=$homeREfta+$val.fta%}
...
<td>&nbsp;</td>
<td>&nbsp;</td>
<td>{%(($homeSTfgm+$homeREfgm)*100/($homeSTfga+$homeREfga))|string_format:"%.1f"%}%</td>
<td>{%(($homeSTtpm+$homeREtpm)*100/($homeSTtpa+$homeREtpa))|string_format:"%.1f" %}%</td>
<td>{%(($homeSTftm+$homeREftm)*100/($homeSTfta+$homeREfta))|string_format:"%.1f"%}%</td>
<td>&nbsp;</td>
<td>&nbsp;</td>
<td>&nbsp;</td>
<td>&nbsp;</td>
...
<a target="_self" href="{%$match_info.boxscore_link|f_escape_xml%}" class="d on"><s></s>{%if $match_info.status == 1%}数据统计{%else if  $match_info.status == 2%}数据直播{%/if%}</a>
...
<a target="_self" href="{%$match_info.textlive_link|f_escape_xml%}" class="b"><s></s>{%if $match_info.status == 1%}文字实录{%else if  $match_info.status == 2%}文字直播{%/if%}</a>
{%if $match_info.status == 1%}
<a target="_self" href="{%$match_info.recap_link|f_escape_xml%}" class="a"><s></s>比赛战报</a>
...
<script type="text/template" id="J_awayTpl">
        <tbody>
            ...
            <tr class="title bg_a" style="background-color: rgb(251, 251, 251);">
                ...
                <td>&nbsp;</td>
                <td>&nbsp;</td>
                <td><@=((awaySTfgm+awayREfgm)*100/(awaySTfga+awayREfga)).toFixed(1)@>%</td>
                <td><@=((awaySTtpm+awayREtpm)*100/(awaySTtpa+awayREtpa)).toFixed(1)@>%</td>
                <td><@=((awaySTftm+awayREftm)*100/(awaySTfta+awayREfta)).toFixed(1)@>%</td>
                <td>&nbsp;</td>
                <td>&nbsp;</td>
                <td>&nbsp;</td>
                <td>&nbsp;</td>
...
<script type="text/template" id="J_homeTpl">
        <tbody>
            ...
            <tr class="title bg_a" style="background-color: rgb(251, 251, 251);">
                ...
                <td>&nbsp;</td>
                <td>&nbsp;</td>
                <td><@=((homeSTfgm+homeREfgm)*100/(homeSTfga+homeREfga)).toFixed(1)@>%</td>
                <td><@=((homeSTtpm+homeREtpm)*100/(homeSTtpa+homeREtpa)).toFixed(1)@>%</td>
                <td><@=((homeSTftm+homeREftm)*100/(homeSTfta+homeREfta)).toFixed(1)@>%</td>
                <td>&nbsp;</td>
                <td>&nbsp;</td>
                <td>&nbsp;</td>
                <td>&nbsp;</td>
...
{%require name='match:page/boxscore.tpl'%}{%/block%}
