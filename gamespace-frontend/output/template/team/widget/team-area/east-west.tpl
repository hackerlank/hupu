...
<td><a href="{%$val.team_link|f_escape_xml%}">{%$val.team_name|f_escape_xml%}</a></td>
<td>{%$val.win|f_escape_xml%}</td>
<td>{%$val.lost|f_escape_xml%}</td>
...
<td class="red_color_font"><a href="/schedule/{%$base_info.eng_name|lower|f_escape_path%}">{%$base_info.name|f_escape_xml%}赛程</a></td>
</tr>
{%if !empty($next_five)%}
{%foreach from=$next_five item=val%}
<tr {%if $val@index%2 ==0%}class="bg_color"{%/if%}>
<td>{%date('m-d', strtotime($val.time))%}</td>
<td><a href="{%$val.home_link|f_escape_xml%}">{%$val.away_name|f_escape_xml%}</a>&nbsp;vs&nbsp;<a href="{%$val.away_link|f_escape_xml%}">{%$val.home_name|f_escape_xml%}</a></td>
<td><a href="{%$val.boxscore_link|f_escape_xml%}">比赛前瞻</a></td>
</tr>
{%/foreach%}
{%else%}
<tr class="bg_color">
...
<td>{%date('m-d', strtotime($val.time))%}</td>
<td><a href="{%$val.home_link|f_escape_xml%}">{%$val.away_name|f_escape_xml%}</a>&nbsp;vs&nbsp;<a href="{%$val.away_link|f_escape_xml%}">{%$val.home_name|f_escape_xml%}</a></td>
<td><a href="{%$val.boxscore_link|f_escape_xml%}">数据统计</a></td>
</tr>
{%/foreach%}
{%else%}
<tr class="bg_color">
...
<div class="video_box">
<div class="title">
<a class="more" target="_blank" href="http://v.liangle.com/nba/tag/{%$base_info.name|f_escape_path%}">更多</a>
{%$base_info.name|f_escape_xml%}视频</div>
<ul>
{%foreach from=$video item=val%}
<li style="background-color: rgb(255, 255, 255);">
<span class="a_1">
<a title="等等" href="{%$val.url|f_escape_xml%}" target="_blank">
