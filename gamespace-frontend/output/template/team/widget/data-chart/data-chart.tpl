...
<a
            ...
            name="{%$player_info.name|f_escape_xml%}"
            arr="{%$data.data_chart[$key_en[$val@index]]|array_to_string:'data'|f_escape_xml%}"
            tit="{%if $type|strpos:'cur' >= 0%}本赛季{%else%}职业生涯{%/if%}{%$key_ch[$val@index]|f_escape_xml%}变化趋势图&nbsp;&nbsp;&nbsp;&nbsp;平均{%$key_ch[$val@index]%}：
                {%if !empty($data.data_chart[$key_en[$val@index]]) %}
                    {%$data.data_chart[$key_en[$val@index]]|avg:'data'%}
                {%else%}
                    0
                {%/if%}
                {%$key_measure[$val@index]%}" pointstart="{%$data.data_chart[$key_en[$val@index]][0].date|f_escape_xml%}"
            measure="{%$key_measure[$val@index]|f_escape_xml%}" categories="{%if !empty($data.data_chart[$key_en[$val@index]])%}
...
<a
            ...
            {%/if%}
            href="javascript:void(0);"
            name="{%$base_info.name|f_escape_xml%}"
            arr="{%$team_data_avg[$key_en_team[$val@index]]|array_to_string:'score'|f_escape_xml%}"
            tit="{%if $type|strpos:'cur' >= 0%}本赛季{%else%}职业生涯{%/if%}{%$key_ch_team[$val@index]|f_escape_xml%}变化趋势图&nbsp;&nbsp;&nbsp;&nbsp;平均{%$key_ch_team[$val@index]%}：
            {%if !empty($team_data_avg[$key_en_team[$val@index]]) %}
                {%$team_data_avg[$key_en_team[$val@index]]|avg:'score'%}
            {%else%}
                0
            {%/if%}
                {%$key_measure[$val@index]%}" pointstart="{%$team_data_avg[$key_en_team[$val@index]][0].time|f_escape_xml%}"
            measure="{%$key_measure[$val@index]|f_escape_xml%}" categories="{%if !empty($team_data_avg[$key_en_team[$val@index]])%}
