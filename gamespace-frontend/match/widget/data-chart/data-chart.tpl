    ...
    <div class="team_qushi">
        {%foreach from=$key_en item=val key=key%}
            {%if !empty($away_latest_five)%}
                ...
                name="{%$match_info.away_name%}/{%$match_info.home_name%}"
                {%if !empty($away_latest_five[$val]) %}
                    arr="{%$away_latest_five[$val]|implode:','%}/{%$home_latest_five[$val]|implode:','%}"
                {%else%}
                    arr="0/0" 
                {%/if%}

                tit="{%$match_info.away_name%}平均{%$key_ch[$val@index]%}:
                {%if !empty($away_latest_five[$val]) %}
                ...
                measure="{%$key_measure[$val@index]%}" categories="5,4,3,2,最近1场">{%$key_ch[$val@index]%}</a>
            {%/if%}
        {%/foreach%}
    </div>
...
{%else%}
    ...
    {%$num = []%}
                    {%foreach from=$home_latest_five[$val] item=item %}
                        {%if $home_latest_five[$item][$item@index] != 0%}
                            {%array_unshift($num, $home_latest_five[$val][$item@index])%}
                        {%/if%}
                    {%/foreach%}
{%/if%}
