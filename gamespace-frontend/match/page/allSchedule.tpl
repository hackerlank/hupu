...
{%block name="head_static"%}
    ...
    {%script%}
        ...
        var base_date = "{%$date[0]%}" || "new Date().toLocaleDateString().replace(/\//g,'-')";
...
<div id="allteamNba" class="gamecenter_livestart">
    ...
    <div class="players_right" style="margin-bottom:10px;">
        ...
        <div class="choosedare clearfix">
            ...
            <div class="a">
                <b><a class="t1" href="/schedule/{%$date[0]|format_date:1:7%}">前七天</a></b><span class="t2">{%$date[0]|date_format:'%m-%d'%} 至 {%$date[1]|date_format:'%m-%d'%}</span><b><a class="t3" href="/schedule/{%$date[1]|format_date:2:1%}">后七天</a></b>
            </div>
            <div class="b"><img src="../static/img/schedule/date.jpg"></div>
            <div id="date">
            </div>
        </div>
            ...
            <tbody>
                ...
                {%foreach from=$match item=val key=key%}
                    <tr class="left linglei">
                        <td colspan="3" class="left" width="135">{%$key|mb_substr:5:2:"utf-8"%}月{%$key|mb_substr:8:2:"utf-8"%}日&nbsp;&nbsp;{%$key|mb_substr:11:3:"utf-8"%}</td>
                    </tr>
                    {%foreach $val as $subVal%}
                    <tr class="left">
                        <td class="left" width="135">{%$subVal.time|date_format:'%H:%M'%}</td>
                        <td width="360">
                            <a href="{%$subVal.away_link%}" target="_blank">{%$subVal.away_name%}</a>&nbsp;vs&nbsp;<a href="{%$subVal.home_link%}" target="_blank">{%$subVal.home_name%}</a>
                        </td>
                        <td>
                            {%if $subVal.status == 1%}
                                <a target="_blank" href="{%$subVal.boxscore_link%}">数据统计</a>&nbsp;&nbsp;&nbsp;<a target="_blank" href="{%$subVal.video_link%}">比赛视频</a>
                            {%else if $subVal.status == 2%}
                                <a target="_blank" href="{%$subVal.boxscore_link%}">数据直播</a>&nbsp;&nbsp;&nbsp;<a target="_blank" href="http://goto.hupu.com/?a=goClick&id=3727">视频直播</a>
                            {%else%}
                                <a target="_blank" href="{%$subVal.boxscore_link%}">比赛前瞻</a>&nbsp;&nbsp;&nbsp;<a target="_blank" href="http://goto.hupu.com/?a=goClick&id=3727">视频直播</a>
                            {%/if%}
                        </td>
                    </tr>
                    {%/foreach%}
