...
{%block name="title"%}
{%if $result.puid == $result.author_puid%}我的{%/if%}论坛回帖{%/block%}
...
<li class="reply-list" data-jump="kanqiu://bbs/topic/{%$val.tid|f_escape_xml%}?page={%$val.post.page|f_escape_xml%}&pid={%$val.post.pid|f_escape_xml%}">
<div class="reply-info">
<div class="reply-time">{%$val.post.time|f_escape_xml%}</div>
<div class="reply-lights">
亮了({%$val.post.light_count|f_escape_xml%})</div>
</div>
...
<h3>来自：<a href="kanqiu://bbs/topic/{%$val.tid|f_escape_path%}">{%$val.title|f_escape_xml%}</a></h3>
</li>
{%/foreach%}
</ul>
...
{%/if%}
{%require name='user:page/myReply.tpl'%}{%/block%}
