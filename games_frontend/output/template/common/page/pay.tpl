...
{%if !empty($paystring)%}
<h3>支付宝code</h3>
<pre>{%$paystring|f_escape_xml%}</pre>
{%/if%}
{%if !empty($payweixinstring)%}
<h3>微信code</h3>
<pre>{%$payweixinstring|f_escape_xml%}</pre>
{%/if%}
...
{%script%}
    window.GM = {
        paystring: "{%$paystring|f_escape_js%}",
        payWeixinString: "{%$payweixinstring|f_escape_js%}"
    };
    ...
    {%/script%}
{%require name='common:page/pay.tpl'%}{%/block%}
