...
{%foreach $menus as $id => $menuInfo}
                            <li style="text-align:center"><a href="./{%$menuInfo['action']|f_escape_xml%}"{%if $menuId == $menuInfo['id']%} class="active"{%/if%}><i class="batch {%$menuInfo['icon']|f_escape_xml%}"></i><br />{%$menuInfo['name']|f_escape_xml%}</a></li>
                            {%/foreach%}
                        </ul>
        ...
        <div class="page">
            <div class="page-container">
                <div class="container">
                    <div class="row">
                        <div class="span2">
                            <span style="display: block; margin: 10px;"><li class="icon-th-large"></li> &nbsp; {%$menus[$menuId]['name']|f_escape_xml%}</span>
                            <div class="sidebar">
                                <ul class="col-nav span2">
                                    {%foreach $menus[$menuId]['subMenu'] as $mid => $menuInfo%}
                                    <li{%if !empty($subMenuId) && $menuInfo['id'] == $subMenuId%}{% $subMenuId = $mid%} class="activeSubMenu"{%/if%}><a href="./{%$menuInfo['action']|f_escape_xml%}"> <i class="pull-right icon-{%$menuInfo['icon']|f_escape_xml%}"></i>{%$menuInfo['name']|f_escape_xml%}</a></li>
                                    {%/foreach%}
                                </ul>
                            </div>
                        </div>
