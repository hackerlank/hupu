...
<input type="text" placeholder="请输入" id="title" value="{%$title|f_escape_xml%}" {%if $is_custom == "0"%}disabled="disabled"{%/if%}/>
</dd>
<dd>
<span class="key">视频地址：</span>
<input type="text" placeholder="URL" id="video-url" value="{%$video_url|f_escape_xml%}" {%if $is_custom == "0"%}disabled="disabled"{%/if%}/>
</dd>
<dd>
<span class="key">视频封面：</span>
<input type="text" placeholder="URL" id="video-cover" value="{%$video_cover|f_escape_xml%}" {%if $is_custom == "0"%}disabled="disabled"{%/if%}/>
</dd>
</dl>
</div>
{%if $is_custom != "0"%}
<div class="recap mod-box">
<strong><i class="icon-tags"></i>&nbsp;&nbsp;添加战报</strong>&nbsp;&nbsp;&nbsp;&nbsp;<a href="javascript:void(0);" class="btn btn-primary" id="J-add-btn">添加</a>
</div>
<div class="mod-box">
...
<tr>
<td>比赛时间：</td>
<td><input type="text" class="match-time" placeholder="请输入" style="width: 50px;" value="{%$val.match_time|f_escape_xml%}"><small>可填数值 1-150整数;0或为空，客户端不显示时间</small></td>
<td rowspan="3"><div class="img"><img src="{%$val.img_url|f_escape_xml%}" class="img-preview" alt="预览图片"></div></td>
</tr>
<tr>
<td>图片地址：</td>
<td><input type="text" class="img-url" placeholder="URL" value="{%$val.img_url|f_escape_xml%}"></td>
</tr>
<tr>
<td>图片大小：</td>
<td><input type="text" class="img-size" placeholder="选填" value="{%$val.img_size|f_escape_xml%}" style="width: 50px;">M<small>(不是GIF图可不填)</small></td>
</tr>
<tr>
<td>文字描述：</td>
<td><textarea class="desc" maxlength="400" placeholder="请输入">{%$val.desc|f_escape_xml%}</textarea><em class="cur_num">{%$val.desc|mb_strlen|f_escape_xml%}</em>/400</td>
...
<dl class="topic-list">
<dt>
<i class="icon-tags"></i>&nbsp;&nbsp;帖子ID：<input type="text" placeholder="帖子ID" id="tid" value="{%$tid|f_escape_xml%}" {%if $is_custom == "0"%}disabled="disabled"{%/if%}><br>
<small>过滤掉有图的亮回帖 比赛结束后的一小时内，每五分钟更新</small>
</dt>
{%foreach $light_reply as $val%}
<dd data-pid="{%$val.pid|f_escape_xml%}" data-ishidden="{%$val.is_hidden|f_escape_xml%}">
<div class="info">
<span class="num">{%$val@index+1%}</span>
<span>昵称:{%$val.userName|f_escape_xml%}</span>
<span>亮了:{%$val.light_count|f_escape_xml%}</span>
</div>
<div class="content">{%$val.content|f_escape_xml%}</div>
...
{%/if%}
<div>
{%require name='admin:page/set_recap_data.tpl'%}{%/block%}
