...
<input type="hidden" id="gid" value="{%$gameInfo.status.id|f_escape_xml%}">
...
<code class="team">{%$gameInfo.home_name|f_escape_xml%}</code>
<code class="vs">{%$gameInfo.home_score|f_escape_xml%}-{%$gameInfo.away_score|f_escape_xml%}</code>
<code class="team">{%$gameInfo.away_name|f_escape_xml%}</code>
<code>{%$gameInfo.status.desc|f_escape_xml%}</code>
</div>
...
<td></td>
<td></td>
<td rowspan="3"><div class="img"><img src="{%$val.url|f_escape_xml%}" class="img-preview" alt="预览图片"></div></td>
</tr>
<tr>
<td>图片地址：</td>
<td><input type="text" class="img-url aaa" placeholder="URL" value="{%$val.url|f_escape_xml%}"></td>
</tr>
<tr>
<td>图片大小：</td>
<td><input type="text" class="img-size" placeholder="选填" value="{%$val.size|f_escape_xml%}" style="width: 50px;">M<small>(不是GIF图可不填)</small></td>
</tr>
<tr>
<td>文字描述：</td>
<td><textarea class="desc" maxlength="100" placeholder="请输入">{%$val.content|f_escape_xml%}</textarea><em class="cur_num">{%$val.content|mb_strlen|f_escape_xml%}</em>/100</td>
<td>
<span class="gifId" style="display:none">{%$val.gifId|f_escape_xml%}</span>
<span class="sort" style="display:none">{%$val.sort|f_escape_xml%}</span>
...
{%/if%}
<div>
{%require name='admin:page/set_game_gifs.tpl'%}{%/block%}
