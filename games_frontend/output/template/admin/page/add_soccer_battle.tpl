...
<div class="player draggable_{%$subVal@index+1%}" data-position="{%$subVal.position|f_escape_xml%}" data-positionname="{%$subVal.position_name|f_escape_xml%}" style="left: {%$subVal.x|f_escape_xml%}px;top: {%$subVal.y|f_escape_xml%}px;">
<div class="player_inner">
{%$subVal.position|f_escape_xml%}<br>
{%$subVal.position_name|f_escape_xml%}
</div>
</div>
{%/foreach%}
{%/if%}
...
{%foreach $data as $key => $val%}
<option value="{%$key|f_escape_xml%}">{%$key|f_escape_xml%}</option>
{%/foreach%}
</select>
<div class="move">
<div>当前位置：</div>
...
<div>
{%require name='admin:page/add_soccer_battle.tpl'%}{%/block%}