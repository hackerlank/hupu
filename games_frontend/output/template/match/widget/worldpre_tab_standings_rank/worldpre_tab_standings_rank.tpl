...
{%if $code != "w174"%}
{%if $val@index == 0%}A{%else if $val@index == 1%}B{%else if $val@index == 2%}C{%else if $val@index == 3%}D{%else if $val@index == 4%}E{%else if $val@index == 5%}F{%else if $val@index == 6%}G{%else if $val@index == 7%}H{%else if $val@index == 8%}I{%/if%}组{%else%}
排行{%/if%}
</td>
...
<tr class="tap {%if !empty($subVal.tid)%}link{%/if%}" {%if !empty($subVal.tid)%}link="kanqiu://soccerleagues/{%$league_name|f_escape_xml%}/team/{%$subVal.tid|f_escape_xml%}"{%/if%}>
<td width="10%" align="center">
{%**
							* 亚洲 w173
