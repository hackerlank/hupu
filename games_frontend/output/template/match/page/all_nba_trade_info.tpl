...
{%if $type == "trade"%}
<tr>
<td width="40%" class="type" colspan="2">{%$chinese|f_escape_xml%}</td>
...
<tr class="link tap" link="kanqiu://nba/nba/team/{%$val.tid|f_escape_xml%}">
<td width="15%" align="center" valign="top">
<span class="logo"><img src="{%$subVal.logo|f_escape_xml%}?t={%$val@index|f_escape_path%}{%$subVal@index|f_escape_path%}"/></span>
</td>
<td width="25%" class="td2" valign="top">{%$subVal.name|f_escape_xml%}</td>
...
{%else%}
{%foreach $data as $val%}
<tr class="link tap" link="kanqiu://nba/nba/player/{%$val.hupu_pid|f_escape_xml%}">
<td>
<table class="data-table">
<tr>
<td width="15%" align="center" {%if $type != "free" && !empty($val.memo)%}rowspan="2" valign="top"{%/if%}>
<span class="logo"><img src="{%$val.photo|f_escape_xml%}?t={%$val@index|f_escape_path%}"/>{%if !empty($night)%}<span class="mask"></span>{%/if%}</span>
</td>
<td width="25%" class="td2">{%$val.alias|f_escape_xml%}</td>
<td width="30%" align="center" class="td3">{%$val.from_team_name|f_escape_xml%}</td>
<td width="30%" align="center" class="td4">
{%if $type == "free"%}
{%$val.memo|f_escape_xml%}
{%else%}
{%$val.to_team_name|f_escape_xml%}
{%/if%}
</td>
</tr>
{%if $type != "free" && !empty($val.memo)%}
<tr>
<td colspan="3" class="contract">{%$val.memo|f_escape_xml%}</td>
</tr>
{%/if%}
</table>
</td>
...
{%require name='match:page/all_nba_trade_info.tpl'%}{%/block%}
