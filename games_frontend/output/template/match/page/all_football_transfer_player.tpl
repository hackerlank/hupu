...
{%script%}
        window.GM = {
            ...
            "league_name": "{%$league_name|f_escape_js%}",
            "rank_type": "{%$rank_type|f_escape_js%}"
        };
...
<thead>
<tr>
<td width="15%" align="center">{%$title|f_escape_xml%}</td>
...
<tbody id="J-all-rank">
{%foreach $data as $val%}
<tr class="link tap" link="kanqiu://soccerleagues/{%$league_name|f_escape_xml%}/player/{%$val.player_id|f_escape_xml%}">
<td width="15%" align="center">{%if $rank_type == "TOP20"%}<span class="top20">{%$val@index+1%}</span>{%else%}{%$val.occur_time|f_escape_xml%}{%/if%}</td>
<td width="11%" align="left">
<span class="logo"><img src="{%$val.photo|f_escape_xml%}?t={%$val@index|f_escape_path%}" width="30" />{%if !empty($night)%}<span class="mask"></span>{%/if%}</span>
</td>
<td width="29%" align="left">
<div class="player-name">{%$val.name|f_escape_xml%}</div>
<div class="old-team">{%$val.from_team_name|f_escape_xml%}(原)</div>
</td>
<td width="20%" align="center">{%$val.to_team_name|f_escape_xml%}</td>
<td width="25%" align="center" {%if !empty($val.memo)%}class="td-icon"{%/if%}>{%$val.fee|f_escape_xml%}{%if !empty($val.memo)%}<i class="icon" data-memo="{%$val.memo|f_escape_xml%}"></i>{%/if%}</td>
</tr>
{%/foreach%}
{%* 前端模板 *%}
...
{%require name='match:page/all_football_transfer_player.tpl'%}{%/block%}
