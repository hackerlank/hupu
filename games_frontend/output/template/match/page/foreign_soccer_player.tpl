{%extends file="common/page/layout.tpl"%} 
{%block name="title"%}
{%if !empty($profile.player_full_name) || !empty($profile.player_enname)%}
{%if !empty($profile.player_full_name)%}
{%$profile.player_full_name|f_escape_xml%}
{%else%}
{%$profile.player_enname|f_escape_xml%}
{%/if%}
{%else%}
饮水机守护神{%/if%}
{%/block%}
{%block name="head_static"%}
{%require name="common:static/js/zepto/zepto.js"%}
{%require name="common:static/js/swiper/js/swiper.jquery.js"%}
{%require name="common:static/js/swiper/css/swiper.css"%}
{%require name="match:static/foreign_soccer_player/foreign_soccer_player.scss"%}
<script type="text/javascript" src="//w1.hoopchina.com.cn/hybrid/common/hpbridge_v1.0.1.js"></script>
{%require name="match:static/foreign_soccer_player/foreign_soccer_player.js"%}
{%script%}
		window.GM = {
            "client": "{%$client|f_escape_js%}" || "x",
            "islogin": !!parseInt("{%$is_login|f_escape_js%}", 10),
            "night":!!parseInt("{%$night|f_escape_js%}", 10),
            "version":"{%$version|f_escape_js%}"||"7.0.7",
            "player_name": "{%$profile.player_full_name|f_escape_js%}" || "",
            "player_enname": "{%$profile.player_enname|f_escape_js%}",
            "isBirth": !!parseInt("{%$profile.is_birthday|f_escape_js%}", 10)
        };

        var birth = require("match:widget/birth/birth.js");
        var localKey = "soccerplayer_" + $(".playerid").val() + "_dropTimes";
        birth.init(localKey);
    {%/script%}
{%/block%}
{%block name="content"%}
<div class="foreign-player-wrap">
<section class="info">
<div class="head">
<img src="{%$profile.player_header|f_escape_xml%}">
{%if !empty($night)%}<div class="mask"></div>{%/if%}
</div>
<ul class="detail">
{%if $team_info|count != 0%}
<li>
{%foreach $team_info as $val%}
{%if $val@index !=0%}<i class="separator">|</i>{%/if%}
<span>{%$val.name|f_escape_xml%}</span>
<span>{%if !empty($val.number)%}{%$val.number|f_escape_xml%}号{%/if%}</span>
{%if $val.is_captain%}
<span class="captain">c</span>
{%/if%}
{%/foreach%}
</li>
{%/if%}
{%if !empty($profile.country)%}
<li>
<span>国籍{%$profile.country|default:"不详"|f_escape_xml%}</span>
</li>
{%/if%}
{%if $profile.height!="" || $profile.weight!="" || $profile.foot!=""%}
<li>
<span>{%if $profile.height!="不详"%}{%$profile.height|f_escape_xml%}{%/if%}</span>
{%if $profile.height!="不详" && $profile.weight!="不详"%}<i class="separator">|</i>{%/if%}
<span>{%if $profile.weight!="不详"%}{%$profile.weight|f_escape_xml%}{%/if%}</span>
{%if $profile.foot!="不详"%}<i class="separator">|</i>{%/if%}
<span>{%if $profile.foot!="不详"%}{%$profile.foot|f_escape_xml%}{%/if%}</span>
</li>
{%/if%}
</ul>
</section>
<ul class="tab" id="J-tab">
<li class="tab-info active">资料</li>
<li class="tab-data">数据</li>
</ul>
<div class="swiper-container">
<div class="swiper-wrapper">
<div class="swiper-slide swiper-info" id="">
<section class="base-info">
<div class="base-item">
<span id="J-birth-date">{%$profile.birth_date|default:"不详"|f_escape_xml%}</span>
<strong>
<i class="birth-status">
<img src="//w1.hoopchina.com.cn/hybrid/resource/emoji/2x/1f382.png?t=1" width="15" height="15">
</i>
{%$profile.age|default:"不详"|f_escape_xml%}
<i class="birth-status">
<img src="//w1.hoopchina.com.cn/hybrid/resource/emoji/1f382.png" width="15" height="15">
</i>
</strong>
</div>
<div class="base-item">
<span>参考身价</span>
<strong>{%$profile.salary|default:"不详"|f_escape_xml%}</strong>
</div>
<div class="base-item">
<span>合同到期</span>
<strong>{%$profile.contract_until|default:"不详"|f_escape_xml%}</strong>
</div>
</section>
</section>
{%if !empty($profile.detailed_position)%}
<section class="position">
<h2>球场位置</h2>
<div class="court-wrap">
<div class="court-bg">
{%if !empty($profile.detailed_position.main_position)%}
<div class="pos pos{%$profile.detailed_position.main_position.type|f_escape_xml%} main">
<span></span>
<em>{%$profile.detailed_position.main_position.name_zh|f_escape_xml%}(主要)</em>
</div>
{%/if%}
{%if !empty($profile.detailed_position.side_position)%}
{%foreach $profile.detailed_position.side_position as $val%}
<div class="pos pos{%$val.type|f_escape_xml%}">
<span></span>
<em>{%$val.name_zh|f_escape_xml%}</em>
</div>
{%/foreach%}
{%/if%}
</div>
<div class="hupu-logo"></div>
</div>
</section>
{%/if%}
{%if !empty($profile.intro) || !empty($profile.depth_profiling)%}
<section class="intro">
{%if !empty($profile.intro)%}
<h2>球员简介</h2>
<p class="intro-con">{%$profile.intro|f_escape_xml%}</p>
{%/if%}
{%if !empty($profile.depth_profiling)%}
<div class="analyze">
<h2>深度剖析</h2>
<ul>
{%foreach $profile.depth_profiling as $val%}
<li>
<a href="{%$val.depth_link|f_escape_xml%}" dace-node="weblinks">
<div class="link-title">
<p class="title">{%$val.depth_title|f_escape_xml%}</p>
<span class="time">{%$val.depth_date|f_escape_xml%}</span>
</div>
<em><i></i></em>
</a>
</li>
{%/foreach%}
</ul>
</div>
{%/if%}
</section>
{%/if%}
{%if !empty($profile.player_video)%}
<section class="video">
<h2>球员视频</h2>
{%foreach $profile.player_video as $val%}
<a href="{%$val.video_link|f_escape_xml%}" class="video-link" dace-node="playervideo">
<div class="video-img">
<em></em>
<img data-src="{%$val.video_thumbnails|f_escape_xml%}" class="swiper-lazy" alt="">
</div>
<p>{%$val.video_title|f_escape_xml%}</p>
</a>
{%/foreach%}
</section>
{%/if%}
<div class="source">
<div>数据支持：</div>
</div>
</div>
<div class="swiper-slide swiper-data" id="">
{%if !empty($team_info[0].name)%}
<section class="season-data">
<h2>{%if $team_info[0].is_country%}{%$team_info[0].league_name|f_escape_xml%}{%else%}{%$team_info[0].season_str|f_escape_xml%}{%/if%}&nbsp;&nbsp;{%$team_info[0].name|f_escape_xml%}</h2>
<div class="data-info">
{%foreach $stats[0].season_data as $val%}
<div class="base-data">
<span>{%$val[0]|f_escape_xml%}</span>
<strong>{%$val[1]|f_escape_xml%}</strong>
</div>
{%/foreach%}
</div>
{%if !empty($stats[0].total_data)%}
<div class="data-table">
<div class="table-title">
<table class="table">
<tr>
<th>赛事</th>
</tr>
{%foreach $stats[0].total_data as $val%}
<tr>
<td>{%$val.block|f_escape_xml%}</td>
</tr>
{%/foreach%}
</table>
</div>
<div class="table-scroll swiper-no-swiping">
<table class="table">
<tr>
{%foreach $stats[0].total_data[0].data as $val%}
<th>{%$val[0]|f_escape_xml%}</th>
{%/foreach%}
</tr>
{%foreach $stats[0].total_data as $key%}
<tr>
{%foreach $key.data as $val%}
<td>{%$val[1]|f_escape_xml%}</td>
{%/foreach%}
</tr>
{%/foreach%}
</table>
</div>
</div>
{%/if%}
</section>
{%/if%}
{%if !empty($team_info[1].name)%}
<section class="season-data">
<h2>{%if $team_info[1].is_country%}{%$team_info[1].league_name|f_escape_xml%}{%else%}{%$team_info[1].season_str|f_escape_xml%}{%/if%}&nbsp;&nbsp;{%$team_info[1].name|f_escape_xml%}</h2>
<div class="data-info">
{%foreach $stats[1].season_data as $val%}
<div class="base-data">
<span>{%$val[0]|f_escape_xml%}</span>
<strong>{%$val[1]|f_escape_xml%}</strong>
</div>
{%/foreach%}
</div>
{%if !empty($stats[1].total_data)%}
<div class="data-table">
<div class="table-title">
<table class="table">
<tr>
<th>赛事</th>
</tr>
{%foreach $stats[1].total_data as $val%}
<tr>
<td>{%$val.block|f_escape_xml%}</td>
</tr>
{%/foreach%}
</table>
</div>
<div class="table-scroll swiper-no-swiping">
<table class="table">
<tr>
{%foreach $stats[1].total_data[0].data as $val%}
<th>{%$val[0]|f_escape_xml%}</th>
{%/foreach%}
</tr>
{%foreach $stats[1].total_data as $key%}
<tr>
{%foreach $key.data as $val%}
<td>{%$val[1]|f_escape_xml%}</td>
{%/foreach%}
</tr>
{%/foreach%}
</table>
</div>
</div>
{%/if%}
</section>
{%/if%}
{%if !empty($profile.transfer_history)%}
<section class="transfer">
<h2>转会纪录</h2>
<div class="table-wrap">
<table>
<tr>
<th>时间</th>
<th>转会至</th>
<th>转会费</th>
</tr>
{%foreach $profile.transfer_history as $val%}
<tr>
<td>{%$val.transfer_date|f_escape_xml%}</td>
<td>{%$val.moving_to_team_name|f_escape_xml%}</td>
<td>
{%if !empty({%$val.loan|f_escape_xml%})%}
{%$val.loan|f_escape_xml%}
{%else%}
{%$val.transfer_fee|f_escape_xml%}
{%/if%}
</td>
</tr>
{%/foreach%}
</table>
</div>
</section>
{%/if%}
{%if !empty($profile.performance)%}
<section class="career-history">
<h2>职业生涯</h2>
<div class="table-wrap">
<table>
<tr>
<th><span>球队</span></th>
<th>进球</th>
<th>助攻</th>
</tr>
{%foreach $profile.performance as $val%}
<tr>
<td>{%$val.team_name|f_escape_xml%}</td>
<td>
<span>
{%if !empty($val.goals)%}
{%$val.goals|f_escape_xml%}
{%else%}
-{%/if%}
</span>
</td>
<td>
<span>
{%if !empty($val.assists)%}
{%$val.assists|f_escape_xml%}
{%else%}
-{%/if%}
</span>
</td>
</tr>
{%/foreach%}
</table>
</div>
<p><em></em><span>{%$desc|f_escape_xml%}</span></p>
</section>
{%/if%}
<div class="source">
<div>数据支持：</div>
</div>
</div>
</div>
</div>
</div>
{%require name='match:page/foreign_soccer_player.tpl'%}{%/block%}
