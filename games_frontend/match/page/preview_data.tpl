		...
		<section class="media" {%if empty($video_room_list) && empty($live_video_list.tv) && empty($live_video_list.huputv) && empty($live_video_list.net)%} style="margin-bottom: 0;"{%/if%}>
			...
			<div class="tv">
				...
				<ul class="clearfix">
					...
					<li>
						<a {%if !empty($val.url)%} href="{%$val.url%}" dace-node="livevideo"{%else%}href="javascript:void(0);"{%/if%} class="item ellipsis {%if empty($val.url)%}no-link{%/if%}">{%$val.name|default:"电视直播{%$val@index+1%}"%}</a>
						{%if !empty($val.pay)%}
							<span class="pay">付费</span>
						{%/if%}
					</li>
