<template>
    <section class="hurt-info mod-rank">
        ...
        <scroller lock-x scrollbar-y>
          <ul>
              <li v-if="datas" v-for="(index, item) in datas" @click="updatePos(index)" :class="{active: index == tabIndex}"><a href="javascript:void(0);">{{item.name}}</a></li>
          </ul>
        </scroller>
		</div>
		...
		<ul>
			<li v-for="(index, item) in datas" class="mod-scroller-con">
				...
				<table class="mod-table">
					<tbody>
						<tr v-for="subItem in item.data" v-if="item.data.length" @click="goto(item)">
							<td width="50%" >
