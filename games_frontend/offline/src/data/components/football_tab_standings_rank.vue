<template>
	<section class="standings-rank">
		...
		<div v-if='leaguename == "chlg" || leaguename == "eurocup" || leaguename == "amerciacup" || leaguename == "uefael"'>
	        ...
	        <div v-for="(key,item) in rank[0].ranks_data" v-if="isInGroups(key)">
				...
				<table class="mod-table">
					<tbody>
						<tr class="tap" v-for="(subIndex, subItem) in item" @click="goto(subItem.tid)">
							<td width="10%">
		...
		<div v-if='leaguename != "chlg" && leaguename != "eurocup" && leaguename != "amerciacup" && leaguename != "uefael" && leaguename != "afccl"'>
		   ...
		   <table class="mod-table">
			   <tbody>
				   <tr v-if="rank[0].ranks_data.length" v-for="(index, item) in rank[0].ranks_data" class="link tap" @click="goto(item.team_id)">
					   <td width="10%">
						   <!-- 意甲 123 45 181920 -->
...
export default {
	...
	methods: {
		...
		isInGroups(index){
			return ~[0,1,2,3,'A','B','C','D','E','F','G','H','I','J','K','L'].indexOf(index)
		},
		getGroup(key){
