...
{%block name="head_static"%}
    ...
    {%script%}
        window.GM = $.extend(window.GM, {
          "today_next": !!parseInt("{%$today_rank_list_next%}", 10),
          "month_next": !!parseInt("{%$month_rank_list_next%}", 10),
          "username": "{%$userInfo.username%}",
          "night": !!parseInt("{%$night%}", 10)
        });

        var rpz_rank = require("app:widget/rpz_rank/rpz_rank.js");
        rpz_rank.init();
    ...
    <section class="rpz-rule" id="J-rpz-rule">
        ...
        <ul>
            ...
            <li>
                <i class="dot"></i>
                <p>
                    只有为球队贡献过人品值的用户才会出现在榜单中。
                </p>
            </li>
            <li>
                <i class="dot"></i>
                <p>
                    日榜：按照日新增人品值数值大小进行排序；数值相同的情况，则人品值高的用户、参与人品赛场次高的用户排在前面。
                </p>
            </li>
            <li>
                <i class="dot"></i>
                <p>
                    月榜：按照人品值数值大小进行排序；数值相同的情况，则日新增人品值高的用户、参与人品赛场次多的用户排在前面。
                </p>
            </li>
            <li>
                <i class="dot"></i>
        ...
        <div class="contact">
            联系方式：<br>
            QQ公众号：800021359；微信号：hupukanqiu
        </div>
    </section>
        ...
        **%}
        <div class="t-content day-rank" id="today_rank_list">
            ...
            <table>
                ...
                <tr>
                    <td class="rank" align="center">{%$val.rank%}</td>
                    <td class="state
                    {%if $val.change == 0%}
                    state-stable
                    ...
                    <td class="name {%if $val.username == $userInfo.username%}mine{%/if%}">{%$val.username%}</td>
                    <td class="score-wrap" align="right">
                        <span class="score {%if $val.win_score >= 0%}score-plus{%else%}score-minus{%/if%}">{%if $val.win_score >= 0%}+{%/if%}{%$val.win_score%}</span>
                        <span class="all {%if $val.score >= 0%}all-positive{%else%}all-negative{%/if%}">{%$val.score%}</span>
                    </td>
                </tr>
                {%/foreach%}
                </tbody>
