    ...
    <section class="my-rpz-wrap">
        ...
        <section class="detail-list">
            <ul id="J-detail-list">
                <li>
                    <dl>
                        <dt>{%$userTopicList.timeFormat.date%}&nbsp;&nbsp;{%$userTopicList.timeFormat.cn%}<span class="cor">人品值</span>
                        {%if $userTopicList.days_score[$userTopicList.day] >= 0%}
                            <span class="score score-plus">+{%$userTopicList.days_score[$userTopicList.day]%}</span></dt>
                        {%else%}
                            <span class="score score-minus">{%$userTopicList.days_score[$userTopicList.day]%}</span></dt>
                        {%/if%}
                    </dl>
                    ...
                    <dd>
                        <div class="left">
                            <div class="home {%if $val.topicInfo.right_option == 1%}home-win{%/if%}">
                                <span class="team">{%$val.topicInfo.option1%}</span>&nbsp;&nbsp;
                                {%if $val.topicInfo.right_option == 1%}<span class="win">胜</span>{%/if%}&nbsp;&nbsp;
                                {%if $val.option == 1%}<span class="checked">已选</span>{%/if%}
                            </div>
                            <div class="away {%if $val.topicInfo.right_option == 2%}away-win{%/if%}">
                                <span class="team">{%$val.topicInfo.option2%}</span>&nbsp;&nbsp;
                                {%if $val.topicInfo.right_option == 2%}<span class="win">胜</span>{%/if%}&nbsp;&nbsp;
                                {%if $val.option == 2%}<span class="checked">已选</span>{%/if%}
                            </div>
                        </div>
                        <div class="right">
                            ...
                            {%else if $val.topicInfo.status == 2%}
                                {%if $val.win_score >= 0%}
                                <span class="score score-plus">+{%$val.win_score%}</span>
                                {%else%}
                                <span class="score score-minus">{%$val.win_score%}</span>
                                {%/if%}
            ...
            <div class="more">
                ...
                <a href="javascript:;" id="J-more-btn" data-more="{%$userTopicList.next%}">加载更多</a>
                {%else%}
