package tv.hupu.utils
{
	 ...
	 * @date 2016-01-21
	 */	
	public class JS2AS extends EventDispatcher
	{
		
		protected static var _instance:JS2AS;
		
		public function JS2AS(priv:privateClass)
		{
		 ...
		 * 
		 */		
		public static function getInstance():JS2AS{
			if(!_instance){
				_instance = new JS2AS(new privateClass());
...
//				var stimer:Timer = new Timer(5000);
//				stimer.addEventListener(TimerEvent.TIMER, function(evt:TimerEvent):void{
////					stimer.reset();
//					_instance.callFromJS({ 
//						"key":"playStream",    //新的流地址
//						"data":{
//							"url":Math.random() > 0.5 ? "rtmp://huputv-ws-live.arenacdn.com/prod/hnDBrQzy9g59EAAK" : "http://lxqncdn.miaopai.com/stream/eCBA42sRx~2LsFbOnUzAWw__.mp4"
////							"url":"rtmp://huputv-ws-live.arenacdn.com/prod/hnDBrQzy9g59EAAK"
////							"url":"http://lxqncdn.miaopai.com/stream/eCBA42sRx~2LsFbOnUzAWw__.mp4"
//						}
//					});
//				});
//				stimer.start();
		...
		public function rcvChat(str:String):void{
			if(str){
				dispatchEvent(new JSInterfaceEvent(JSInterfaceEvent.RECEIVE_MSG, str));
			}
		}
		...
		public function rcvSelfChat(str:String):void{
			if(str){
				dispatchEvent(new JSInterfaceEvent(JSInterfaceEvent.RECEIVE_SELF_MSG, str));
			}
		}
		 ...
		 * 
		 */		
		public function playStream(data:Object):void{
			//ConsTrace.htrace('playVod: ' + JSON.stringify(data));
			if(data && data.hasOwnProperty('url')){
				dispatchEvent(new JSInterfaceEvent(JSInterfaceEvent.RECEIVE_STREAM_ADDR, data));
			}
		}
...
//			ConsTrace.htrace('rcvSkillStat: ' + data.team_id, on);
			if(data && data.hasOwnProperty('skill_logo') && data.hasOwnProperty('team_id')){
				dispatchEvent(new JSInterfaceEvent(on ? JSInterfaceEvent.RECEIVE_SKILL_STAT_ON : JSInterfaceEvent.RECEIVE_SKILL_STAT_OFF, data));
			}
		}
		...
		public function rcvGift(obj:Object):void{
			if(obj && obj.hasOwnProperty('position')){
				switch (int(obj.position)){
					case 1:	//特殊礼物，从右上角弹出
						dispatchEvent(new JSInterfaceEvent(JSInterfaceEvent.RECEIVE_SPECIAL_GIFT, obj));
						break;
					case 2:	//人浪
						dispatchEvent(new JSInterfaceEvent(JSInterfaceEvent.RECEIVE_WAVE_GIFT, obj));
						break;
					default:
						break;
				}
		...
		public function getBeans(msg:String):void{
			dispatchEvent(new JSInterfaceEvent(JSInterfaceEvent.RECEIVE_GET_BEANS, msg));
		}
		
		public function getBeansResult(msg:String):void{
			dispatchEvent(new JSInterfaceEvent(JSInterfaceEvent.RECEIVE_GET_BEANS_RESULT, msg));
		}
