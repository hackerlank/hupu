package tv.hupu.providers{
    ...
    public class RTMPVideoProvider extends EventDispatcher implements IProvider{
		...
		private var _lastns:NetStream;
        private var _rtmpRetryTimer:Timer;
        private var _ncRTMPRetryThreshold:int = 15;
        ...
        private function initNetStream():void{
			var _isFirstNs:Boolean = _ns == null;
			if(!_isFirstNs){
				_lastns = _ns;
			}
			...
			if(_isFirstNs){	//如果是第一次播放，直接添加到当前video，否则等缓冲好了再添加到当前video
				_videoReference.attachNetStream(_ns);
			}
//			ConsTrace.htrace("initNetStream: " + _src.streamURL + "/" + _videoReference.name);
        ...
        private function onNetConnectionStatus(e:NetStatusEvent):void{
			ConsTrace.htrace("onNetConnectionStatus: " + e.info.code);
            switch(e.info.code){
        ...
        private function onNetStreamStatus(e:NetStatusEvent):void{
			ConsTrace.htrace("onNetStreamStatus: " + e.info.code);
            switch(e.info.code){
                ...
                case "NetStream.Buffer.Full":
					...
					if(_lastns != null){
						_lastns.close();
						_lastns.removeEventListener(NetStatusEvent.NET_STATUS, onNetStreamStatus);
						_lastns = null;
						_videoReference.attachNetStream(_ns);
					}
                    break;
