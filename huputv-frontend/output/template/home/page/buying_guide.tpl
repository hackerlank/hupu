...
<script>
		var restCoin = '{%$balance["6"]|f_escape_js%}',
			isEnough = '{%$balance["6"] > $bears_recharge_step[$bears_recharge_step|count - 1]%}',
			recharge_url = '{%$recharge_url|f_escape_js%}',
			referer = '{%$referer|f_escape_js%}';
...
<span class="span_2" id="J_bean">{%$balance["1"]|f_escape_xml%}</span>
</div>
<div class="buying_top_r">
<span class="span_1">虎扑币余额</span><span class="span_2" id="J_coin">{%$balance["6"]|f_escape_xml%}</span>
</div>
</div>
...
<label for="beans_{%$val|f_escape_xml%}">
<input 
								name = "beans" 
								type = "radio" 
								value = "{%$val|f_escape_xml%}" 
								class = "c_radio" 
								id = "beans_{%$val|f_escape_xml%}" 
								data-index = "0"
								{%if ($val > $balance["6"])%}disabled{%/if%}
							/>
{%$val|f_escape_xml%}金豆</label>
</li>
{%/foreach%}
</ul>
</div>
...
{%require name='home:page/buying_guide.tpl'%}{%/block%}