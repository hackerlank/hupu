...
<span class="beans">
x{%$task.online.cur_score|f_escape_xml%}
</span>
<span class="text">
<span>领取</span>
</span>
...
<span class="text">
<span class="btn-timing">{%$task.online.rest_time_str|f_escape_xml%}</span>
<span>x{%$task.online.cur_score|f_escape_xml%}</span>
</span>
</a>
{%/if%}
{%/if%}
...
<span>签到领金豆</span>
<em>已连续签到<em id="J_signDays">{%$task.sign.keep_days|f_escape_xml%}</em>天</em>
</div>
<div class="mask-tab J_onlineTab {%if $task.online.status == 2%}notice{%/if%}">
<span>在线领金豆</span>
<em class="tab-timing">
{%if $task.online.status == 1%}
{%$task.online.rest_time_str|f_escape_xml%}后可领取{%elseif $task.online.status == 2%}
可领取{%elseif $task.online.status == 3%}
已完成{%/if%}
</em>
</div>
<div class="mask-tab J_shareTab {%if $task.share.status == 1%}notice{%/if%}">
<span>分享有礼</span>
<em>还可领取<em class="J_shareTimesRest">{%$task.share.rest_times|f_escape_xml%}</em>次</em>
</div>
</div>
...
<div class="sign-tip">
今日已签到，获得<span id="J_signBeans">{%$task.sign.now_bean|f_escape_xml%}</span>个金豆</div>
<a href="javascript:;" class="sign-btn signed" id="J_signBtn">已签到</a>
{%else%}
<div class="sign-tip">
你已连续签到<span>{%$task.sign.keep_days|f_escape_xml%}</span>天</div>
...
<div class="beans-num {%$beanClass[$val@index]|f_escape_xml%}">
{%$beansNum[$val@index]|f_escape_xml%}
</div>
{%if $val.status == 0%}
<a href="javascript:;" class="online-btn wait">等待中</a>
{%elseif $val.status == 1%}
<a href="javascript:;" class="online-btn disable item-timing">{%$task.online.rest_time_str|f_escape_xml%}</a>
{%elseif $val.status == 2%}
...
<div class="share-tip">
你已领取<span>{%$task.share.times|f_escape_xml%}</span>次金豆，还可以领取<span>{%$task.share.rest_times|f_escape_xml%}</span>次。</div>
...
Task.init({%$chaoneng|default:false|f_escape_js%});
