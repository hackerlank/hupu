...
<h1 class="logo">
<a href="/" title="亮了网">
<img src="http://b1.hoopchina.com.cn/web/tv/static/common/widget/header/img/logo_7ecaadf.png" alt=""></a>
</h1>
<div class="nav">
{%if !empty($category) %}
<ul>
...
{%foreach $category as $val%}
{%if !empty($val.display)%}
<li><a href="{%$val.abs_url|f_escape_xml%}" dace-node="nav_channelClick" {%if isset($category_id) && $category_id == $val.sid && $current_page!="center"%}class="active"{%/if%}>{%$val.sname|f_escape_xml%}</a></li>
{%/if%}
{%/foreach%}
</ul>
{%/if%}
...
{%else%}
<dt><span>{%$open_soon.list[0].anchor_nickname|f_escape_xml%}</span>&nbsp;已开播</dt>
{%/if%}
{%foreach $open_soon.list as $val%}
<dd>
<a href="{%$val.abs_url|f_escape_xml%}" title="{%$val.room_name|f_escape_xml%}" dace-node="nav_myfollowRoomClick">
<div class="cover">
<img src="{%$val.cover|f_escape_xml%}" width="78" height="45" alt="">
</div>
<div class="text">
<h3 class="room-name mod-overflow">{%$val.room_name|f_escape_xml%}</h3>
<div class="info">
<div class="user fl">
主播：<span>{%$val.anchor_nickname|f_escape_xml%}</span>
</div>
{%if !empty($show_online)%}
<div class="audience fr">
<i class="icon-audience"></i>
<span>{%$val.online|f_escape_xml%}</span>
</div>
{%/if%}
</div>
</div>
...
<span class="head"><img src="{%$user.avatar.header_small|f_escape_xml%}" alt=""></span>
<span class="name">{%$user.username|f_escape_xml%}</span>
</a>
<ul class="user-list">
...
<li class="last-item"><a href="{%$logout_url|f_escape_xml%}">退出登录</a></li>
</ul>
</span>
</div>
{%else%}
