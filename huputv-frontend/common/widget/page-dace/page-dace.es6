...
module.exports = {
     ...
     */
     sendArenaCloudEvent(path, param) {
        if(window.XMLHttpRequest && window.HTV){
            var isMobile = /(android|ip(hone|od|ad))/i.test(navigator.userAgent);
            var streams = HTV.flashStreamAddress ? HTV.flashStreamAddress() : [];
            var streamurl = isMobile ? document.getElementsByTagName('source')[0].getAttribute('src') : (streams.length ? streams[0]['rtmp'] : '');
            var commonParam = {
                "head": {
                    "streamId": HTV.roomID,           //流ID or 房间ID    （新模式huputTv自己填充）
                    "streamName": document.title,         //流名称 or房间名称   （新模式huputTv自己填充）
                    "platform": isMobile?'h5':'pc',          //平台 ios/android/pc/h5
                    "time": Math.floor(new Date().getTime()/1000),      //秒时间戳
                    "isRegister": !!HTV.isLogin,      //秒时间戳
                    "streamUrl": streamurl,          //流URL
                    "token": this.getCookie('__dacevid3') || this.getCookie('_dacevid3'),             //用户唯一标识
                    "deviceNo": this.getCookie('__dacevid3') || this.getCookie('_dacevid3')             //用户唯一标识，使用dacevid
                }
            };
         ...
         switch(type) {
             ...
             case 1:
                this.sendArenaCloudEvent('playHeartbeat', {"body":{"duration": value}});
                 arr = ["{source}播放时长", "{source}_时长_播放" , "{source}_时长_播放_" + HTV.roomID, "{value}"];
                 break;
             // 加载时长
             case 2:
                this.sendArenaCloudEvent('playCatonEnd', {"body":{"duration": value}});
                 arr = ["{source}加载时长", "{source}_时长_加载" , "{source}_时长_加载_" + HTV.roomID, "{value}"];
                 break;
             default:
                 // 卡顿
                 if(value == '卡顿'){
                    this.sendArenaCloudEvent('playCatonStart', {"body":{}});
                     arr = ["{source}卡顿次数", "{source}_次数_卡顿" , "{source}_次数_卡顿_" + HTV.roomID, 1];
                 // 错误
                 }else if(/错误/.test(value)){
                    this.sendArenaCloudEvent('playerror', {"body":{"failMessage":value}});
                     arr = ["{source}错误次数", "{source}_次数_" + value, "{source}_次数_错误_" + HTV.roomID, 1];
                 }else{
                     arr = ["{source}{value}点击次数", "{source}_点击次数_{value}", "{source}_点击次数_{value}_" + HTV.roomID, 1];
                 }
